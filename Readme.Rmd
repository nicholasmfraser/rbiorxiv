...
The main functions in `biorrxiv` generally conform to the API endpoints outlined in the API documentation ([see here](https://api.biorxiv.org/)).
...
library(viridis) # Nice color-blind friendly scales

# Note that there some irregularities in the case of category names - here we
# convert all categories to title case for consistency
biorxiv_content(from = "2013-11-01", to = "2019-12-31", 
                limit = "*", format = "df") %>%
  filter(version == 1,
         category != "") %>%
  mutate(date = floor_date(date(date), unit = "month"),
         category = str_to_title(category)) %>%
  group_by(date, category) %>%
  summarize(n = n()) %>%
  ggplot() +
  geom_bar(aes(x = date, y = n, fill = category),
